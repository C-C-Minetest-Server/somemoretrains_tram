...
local function tram_set_line(self, data, train)
	...
	if line then
		if string.len(line) == 1 or line == "X" then
			...
			string.format(
			"^(somemoretrains_tram_line.png^[colorize:#%X%X%X%X%X%X)^(somemoretrains_tram_line%s_.png^somemoretrains_tram_line_%s.png",
				math.floor(red / 16), math.fmod(red, 16), math.floor(green / 16), math.fmod(green, 16),
				math.floor(blue / 16), math.fmod(blue, 16), string.sub(line, 1, 1), string.sub(line, 2, 2))
...
advtrains.register_wagon("tram", {
	...
	custom_on_velocity_change = function(self, velocity, old_velocity, dtime)
		...
		if velocity > 0 and (self.sound_loop_tmr or 0) <= 0 then
			self.sound_loop_handle = minetest.sound_play({ name = "somemoretrains_tram_loop", gain = 0.3 },
				{ object = self.object })
